...
glimpse(cze_core)
# get "Political" table for the Austrian Nationalrat ------------------------------------------
...
round((female_share*100), 2) # fema1e share of legislators in Czech Poslanecka Snemovna is 19.48%
...
cze_core_2 <- cze_core |>
  ...
  mutate(
    age_at_death = (difftime(death, birth, units = "auto")/365)|>
      as.numeric() |>
      round(0)
  )
...
fig1 <- ggplot(cze_core_2, 
  ...
  theme_minimal() +
  labs(title = "Fig 1: Distribution of Austrian Legislators' Age at Death",
...
While perfectly plausible, this seems perhaps too convenient and leads me to wonder if there were issues with the data collection - especially as summer birthdays are statistcally more likely (in Europe). Perhaps 01 January was used as a placeholder if bday was unknown? I'll examine the 2nd most frequent birthday in case.
...
This perhaps might in fact be the true value given the data deficiencies here. Without more information it is not possible to make a concrete conclusion.
Given the most frequent value was 01-01, I'm also slightly suspicious of this value being the 1st of the month (again, perhaps if granular data on the day was unavailable for whatever reason, the first of the month was used). For this reason, I'll also take a quick look at the 3rd most frequent date.
...
# this posited phenomenon did not hold up. It is interesting that 2nd and 3rd values are 1st of the month... possible data collection issue?
```

While both the others are completely plausible, this is certainly not a placeholder. 
...
# alternatively use slice functions
cze_core_4 |>
...
NB there have only been 9 sessions, this seems suprisingly few for a dataset going back to 1992
...
And there have been 29 unique values for both start & end dates of the session. Worth checking if sessions is correctly coded.
...
fig2
```
This timeline shows that in fact the session variable is accurate, and that there have only been 9 unique sessions. The 29 figure from the start/end date measure reflects a data collection issue for the final two sessions.
...
# this ranks parties by the amount they appear throughout the dataset across all sessions; I did this because some parties appear and disappear according to session, so I want to stabilise the units I am examining.

parties_to_include <- names(parties_prop_ungrouped[1:5])
parties_to_include
# this returns a vector of the 5 most 'influential' parties ('influential' understood as having persistent representation... this is of course a very contentious definition, but will do for the purpose of this analysis)
...
2) It's also the case that none of the parties exhibit perfect perseverance, and so when each of them fails to show up in a session it is important not to interprete this as a lack of female legislators for that party for that session, but a lack of overall party presence for that session. 
In fact, in this sense, it would be better just to look at female share of the legislature as a whole, not conditioning on party given some of the misleading tendenceis of the above. 

Nevertheless, it is best to treat Figure 6 as a complement to Figure 3 which gives a more accurate overall impression of the phenomenon and use Figure 6 to supplement that understanding with a more granular party-based understanding. With that in mind we can say that there isn't a dramatic change in female representation amongst the most persistent parties, and so the trend seen in Figure 3 must by inference be driven by external parties. Ultimately there's not enough data points and too much variance for this to be statistically significant.
...
#total_legis <- cze_combo |>
  #group_by(session) |>
  #count() 

#parties_prop_grouped <- cze_combo_3 %>%
  #group_by(session, party) %>%
  #count() %>%
  #rename(legis_count_by_party = n) %>%
  #mutate(prop_count_by_party = legis_count_by_party / 218)
...
#parties_prop_grouped$prop_count_by_party <- parties_prop_grouped$legis_count_by_party / 218

#female_share_df <- female_share_df |>
 # left_join(parties_prop_grouped, by = c("session"))
